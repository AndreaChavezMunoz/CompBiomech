function [output] = equilibrium(Ri, Ro, lambda, Pi, materialParameters, x0)
%{
Input Parameters:
Ri: Reference inner radius.
Ro: Reference outer radius (guess).
...
Output:
output: A 1x3 array containing the minimizng value, the computed pressure and force.

%}

ro = x0(1,1); % outer radius

% calculate the current inner radius 
...
fval = P - Pi; % ultimately fvalue needs to go to zero (within the defined tolerance)
force = F;
pressure = P;
output = [fval, pressure, force];
